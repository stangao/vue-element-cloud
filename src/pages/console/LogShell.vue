...
<template lang="html">
    ...
    <div v-title>{{title}}</div>
      ...
      <ul>
        ...
        <li>
         项目名<input class="project" v-model="project"/> Pod名<input class="pod" v-model="pod"/> Container名<input class="container" v-model="container"/>  <router-link :to="detailUrl">去日志界面</router-link>
        </li>
      </ul>
  ...
  data() {
    ...
    computed:{
       detailUrl(){
         ...
         var url =  "/logDetail/"+encodeURIComponent(namespace)+ "/"+ encodeURIComponent(this.token)+"/"+ encodeURIComponent(this.container)
         console.log(url)
         return url
       }
